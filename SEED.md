...
- System tools: `curl`, `tar`, `python3`, `git` (`git` pulls the shared `ld-shared` layer), `docker` (the installer execs `hermes cron` inside the container via `docker compose exec`). No install needed. (No `jq`: the Pi's package set deliberately omits it, so the ld-config assembly and gate use `python3`.)
...
- **PII never leaks.** The owner name input and the derived calendar account are personal-context-secret. They MUST NOT be echoed to stdout, MUST NOT be written anywhere in the SEED tree, and MUST reach the `python3` assembler only **through the environment (`os.environ`), never argv**. The Plow connector token used to derive the account is likewise never echoed — it is passed to `plow_connector.py` through the environment. Only the non-PII `family.timezone` MAY be passed as an argv argument. The assembled config is JSON-validated AND run through the [minimal structural gate](#minimal-structural-gate) BEFORE the atomic `mv`; a blank input or a gate failure FAILS LOUD, non-zero, with nothing landed.
...
3. **Skills installed and wrappers executable.** Do all seven `ld-*` markers exist under `<scaffold>/data/skills/` — each producer's `SKILL.md` (or, for `ld-shared`, `scripts/post_to_kiosk.py`) — AND does every installed producer `post_*.py` wrapper carry the executable bit (mode +x), so the documented "run the helper by absolute path" invocation works rather than failing `Permission denied` (exit 126)? Expected: yes.
