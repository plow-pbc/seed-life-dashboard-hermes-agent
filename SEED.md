...
- **family.timezone is autodetected, not an input.** The IANA zone is everything after the last `/zoneinfo/` in `readlink /etc/localtime`, falling back to `America/Los_Angeles` when detection yields nothing. This is one inline `readlink` + parse.
- **A detected timezone conflict MUST be resolved by ONE targeted confirmation BEFORE assembly.** When an independent signal the installer already holds implies a different IANA zone than the host's — the canonical case: `LD_OWNER_IMESSAGE` is an E.164 NANP number whose area code maps elsewhere — the installer MUST NOT assemble on the host zone. It asks one targeted confirmation naming both candidates and exports the answer as `LD_TIMEZONE` before running the [install block](#requirements); `ref/install-skills.sh` honors a pre-set `LD_TIMEZONE` and autodetects only when it is unset. No conflict → no question.
